...
from hummingbot.connector.exchange.elixir import (
    elixir_constants as CONSTANTS,
    elixir_utils as utils,
    elixir_web_utils as web_utils,
)
...
if TYPE_CHECKING:
    from hummingbot.connector.exchange.elixir.elixir_exchange import ElixirExchange


class ElixirAPIOrderBookDataSource(OrderBookTrackerDataSource):

    _logger: Optional[HummingbotLogger] = None

    def __init__(self,
                 trading_pairs: List[str],
                 connector: 'ElixirExchange',
                 api_factory: WebAssistantsFactory):
        super().__init__(trading_pairs)
        self._connector: ElixirExchange = connector
    ...
    async def listen_for_order_book_snapshots(self, ev_loop: asyncio.AbstractEventLoop, output: asyncio.Queue):
        """
        Elixir sends always full snapshots through the depth channel. That is why they are processed here.
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_timestamp: float = int(snapshot_data["receipt_timestamp"]) * 1e-3
        update_id: int = int(snapshot_data["update_id"])
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        for trade_data in trade_updates:
            ...
            trade_message: Optional[OrderBookMessage] = OrderBookMessage(
                ...
                timestamp=int(trade_data["trade_id"]))

            message_queue.put_nowait(trade_message)

    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        # Elixir never sends diff messages. This method will never be called
        pass

    async def _parse_order_book_snapshot_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        book_updates: Dict[str, Any] = raw_message["data"]

        for book_data in book_updates:
            timestamp: float = int(book_data["receipt_timestamp"]) * 1e-3
            update_id: int = int(book_data["update_id"])
            ...
            order_book_message_content = {
                "trading_pair": trading_pair,
                "update_id": update_id,
                "bids": [(bid[0], bid[1]) for bid in book_data["bids"]],
                "asks": [(ask[0], ask[1]) for ask in book_data["asks"]],
            }
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        try:
            ...
            payload = {
                ...
                "params": {
                    "exchange": self._connector.protocol_exchange,
                    "channel": CONSTANTS.TRADE_CHANNEL_NAME,
                    "symbols": symbols
                }
            }
            ...
            payload = {
                ...
                "params": {
                    "exchange": self._connector.protocol_exchange,
                    "channel": CONSTANTS.DEPTH_CHANNEL_NAME,
                    "symbols": symbols
                }
            }
    ...
    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        ...
        if "data" in event_message:
            ...
            if event_channel == CONSTANTS.TRADE_CHANNEL_NAME:
                channel = self._trade_messages_queue_key
            if event_channel == CONSTANTS.DEPTH_CHANNEL_NAME:
